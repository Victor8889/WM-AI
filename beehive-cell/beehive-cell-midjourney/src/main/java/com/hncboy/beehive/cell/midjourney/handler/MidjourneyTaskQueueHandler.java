...
public class MidjourneyTaskQueueHandler {
     ...
     * 消费记录 key 前綴
     */
    public static final String RECORD_MJ_KEY = PREFIX + "record:mj:";

    /**
     * 用于分布式锁的 消费记录 key
     */
    private static final String RECORD_LOCK_KEY = PREFIX + "lock";

    @Lock4j(name = RECORD_LOCK_KEY, expire = 60000, acquireTimeout = 3000)
    public boolean pushNewRecord(String taskId,int userId,String model){
        try {
            //啥也不处理直接插入
            // 插入队列尾部
            if(!RedisUtil.hasKeyValue(RECORD_MJ_KEY, String.valueOf(taskId + "," + userId + "," + model)))
                RedisUtil.lRightPush(RECORD_MJ_KEY, String.valueOf(taskId + "," + userId + "," + model));
        }catch (Exception e){
            return false;
        }
        return true;
    ...
    public List<String> pullAllRecords(){
        try {
            //啥也不处理直接获取所有的
            return RedisUtil.lRange(RECORD_MJ_KEY, 0,-1);
        }catch (Exception e){
            return null;
        }
    }
    @Lock4j(name = RECORD_LOCK_KEY, expire = 60000, acquireTimeout = 3000)
    public long removeRecord(String value){
        try {
            //啥也不处理直接获取所有的
            return RedisUtil.delete(RECORD_MJ_KEY, value);
