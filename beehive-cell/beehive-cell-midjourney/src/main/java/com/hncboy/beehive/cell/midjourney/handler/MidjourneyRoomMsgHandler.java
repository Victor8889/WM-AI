...
public class MidjourneyRoomMsgHandler {
    ...
    public static void checkExistProcessingTask() {
        ...
        long count = roomMidjourneyMsgService.count(new LambdaQueryWrapper<RoomMidjourneyMsgDO>()
                ...
                // 进行中的状态
                .in(RoomMidjourneyMsgDO::getStatus, MidjourneyMsgStatusEnum.SYS_QUEUING,MidjourneyMsgStatusEnum.SYS_WAIT, MidjourneyMsgStatusEnum.MJ_WAIT_RECEIVED, MidjourneyMsgStatusEnum.MJ_IN_PROGRESS));
        if (count >= 6) {
            throw new ServiceException("单用户最多同时执行6个 Midjourney 任务，请等待任务执行完毕后再试");
        }
    }
    ...
    public static void checkCanUpscale(RoomMidjourneyMsgDO parentRoomMidjourneyMsgDO, int index, MidjourneyProperties midjourneyProperties) {
        ...
        if (isUse) {
            throw new ServiceException("该图片已经进行过 U" + index);
        }
