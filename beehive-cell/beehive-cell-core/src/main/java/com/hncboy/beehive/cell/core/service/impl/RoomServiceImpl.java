...
import com.hncboy.beehive.cell.core.domain.request.RoomAddRequest;
...
public class RoomServiceImpl extends ServiceImpl<RoomMapper, RoomDO> implements RoomService {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public RoomListVO addRoom( RoomAddRequest roomAddRequest){

        // 校验 Cell 是否有使用权限
        //CellPermissionHandler.checkCanUse(CellHandler.convertData(roomAddRequest.getCellConfigCode()));

        // 检查房间配置项参数请求
        //List<RoomConfigParamBO> roomConfigParamBOList = RoomConfigParamHandler.checkRoomConfigParamRequest(CellHandler.convertData(roomAddRequest.getCellConfigCode()), roomCreateRequest.getRoomConfigParams(), false)
        //        // 过滤出用户自己填的
        //        .stream().filter(bo -> !bo.getIsUseDefaultValue()).toList();
        ...
        RoomDO roomDO = new RoomDO();
        roomDO.setUserId(FrontUserUtil.getUserId());
        roomDO.setColor(roomAddRequest.getColor());
        roomDO.setName(roomAddRequest.getName());
        roomDO.setPinTime(0L);
        roomDO.setCellCode(CellHandler.convertData(roomAddRequest.getCellConfigCode()));
        roomDO.setIsDeleted(false);
        save(roomDO);
