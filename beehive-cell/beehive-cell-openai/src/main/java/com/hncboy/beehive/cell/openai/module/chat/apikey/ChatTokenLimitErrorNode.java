...
public class ChatTokenLimitErrorNode implements ChatErrorNode {
    ...
    @Override
    public Pair<Boolean, String> doHandle(RoomOpenAiChatMsgDO questionMessage,Boolean enabled) { //Map<OpenAiChatCellConfigCodeEnum, DataWrapper> roomConfigParamAsMap
        ...
        if (OpenAiChatApiModelEnum.maxTokens(modelName) <= promptTokens) {
            ...
            // 获取当前 prompt 消耗的 tokens   OpenAiChatApiModelEnum.NAME_MAP.get(modelName).getCalcTokenModelName()
            int currentPromptTokens = TikTokensUtil.tokens(CommonEnum.NAME_MAP_PRODUCT.get(modelName) .getApiModel(), questionMessage.getContent());
            // 判断历史上下文是否超过限制
