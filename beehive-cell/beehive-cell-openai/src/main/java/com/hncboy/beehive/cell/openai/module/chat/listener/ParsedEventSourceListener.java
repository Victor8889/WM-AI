...
public class ParsedEventSourceListener extends EventSourceListener {
    ...
    private ChatErrorNode chatErrorNode = SpringUtil.getBean(ChatLocalSensitiveWordErrorNode.class);
    private RoomOpenAiChatMsgDO questionMessage = new RoomOpenAiChatMsgDO();
    ...
    public void onEvent(@NotNull EventSource eventSource, String id, String type, @NotNull String originalData) {
        ...
        str = roomOpenAiChatMsgVO.getContent();
        questionMessage.setContent(str);
        Pair<Boolean, String> errorHandleResultPair = chatErrorNode.doHandle(questionMessage, true);
        if (!errorHandleResultPair.getKey()) {
            //获取敏感词
            words = errorHandleResultPair.getValue();

            //System.out.println("`````````````````````````"+words);
            mutableList = Arrays.asList(words.split(","));;
        ...
        roomOpenAiChatMsgVO.setContent(str);
        chatMessageStorage.setReceivedMessage(str);
        // 遍历监听器
        for (AbstractStreamListener listener : listeners) {
            if (isEnd) {
    ...
    public void onFailure(@NotNull EventSource eventSource, @Nullable Throwable t, @Nullable Response response) {
        ...
        if(StringUtil.isNotBlank(responseStr)) {
            ...
            chatMessageStorage.setErrorResponseData(t.getMessage());
            dataStorage.onError(chatMessageStorage);
        }
