...
public class FrontUserExtraBindingServiceImpl extends ServiceImpl<FrontUserExtraBindingMapper, FrontUserExtraBindingDO>
    ...
    public Integer getEmailUserId() {
        FrontUserExtraBindingDO fu = this.getOne(new LambdaQueryWrapper<FrontUserExtraBindingDO>()
                .eq(FrontUserExtraBindingDO::getBaseUserId, FrontUserUtil.getUserId()));
        if(null == fu)
        return -1;
        else
            return fu.getExtraInfoId();
    }
}
