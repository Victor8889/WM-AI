...
public class BaiduAipHandler {
     ...
     * @return 审核是否通过
     */
    public static Pair<Boolean, String> isCheckImgPass(String identify, String url) {
        // 实时查询缓存判断是否启用
        if (!BaiduAipUtil.getBaiduAipProperties().getEnabled()) {
            return new Pair<>(true, null);
        }
        ...
        long currentTime = DateUtil.current();
        log.info("审核标识：{}，图片审核，开始审核时间：{}, 审核内容：{}", identify, currentTime, url);
        Map<String, Object> resultMap = SpringUtil.getBean(AipContentCensor.class).imageCensorUserDefined(url, EImgType.URL, null).toMap();
        log.info("审核标识：{}，图片审核，审核总时长：{} 毫秒，审核结果：{}", identify, DateUtil.spendMs(currentTime), resultMap);
