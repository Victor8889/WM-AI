...
public interface BeehiveBaseMapper<T> extends BaseMapper<T> {
    ...
    default List<T> cursorList(CursorQuery cursorQuery, SFunction<T, ?> columnFunction, @Param(Constants.WRAPPER) LambdaQueryWrapper<T> queryWrapper) {
        queryWrapper.apply("status != {0}", RoomOpenAiChatMsgStatusEnum.DELETE_CHAT);
        // 多少条记录
     ...
     * @return 查询结果列表
     */
    default List<T> cursorQrList(CursorQuery cursorQuery, SFunction<T, ?> columnFunction, @Param(Constants.WRAPPER) LambdaQueryWrapper<T> queryWrapper) {
